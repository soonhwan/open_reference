	...
	function create(sel){
		...
		if(!sel.multiple){//normal
			...
			ac.onclick=function(){
				...
				//20141030 SMS 추가로 인한 수정
				if($('.smsholder').size() > 0){
					$('.smsholder').each(function(){
						if($(this).find('.layer_pop').size() == 0){
							$(this).hide();
						}
					})
				}
				...
				setTimeout(function(){
					...
					_this.opt.style.top= top + _this.offsetHeight+v.option.position + 'px';
				},100);
