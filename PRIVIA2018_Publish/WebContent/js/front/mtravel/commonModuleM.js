...
var pvmFrontScript = window.pvmFrontScript || (function(){
	return {
		init: function(){
			...
			pvmFrontScript.comHeaderControl(); 

			//컨텐츠 스크립트 모음
			pvmFrontScript.comContents();
		},
		...
		comSearchEvtBind: function($section){
			...
			if(section.find('.ui-capacity .b-cancel a').length > 0){
				section.find('.ui-capacity .b-cancel a').on('click', function(e){
					pvmFrontScript.docuMoudownTrigger();
					e.preventDefault();
				});
			}
		...
		comSearchRecently: function($section){
			...
			//event 바인딩
			pvmFrontScript.comSearchEvtBind($section.find('.hss-recently-search'));
		},
		...
		onHeaderPromRoll: function(){
			/* 설명   : 헤더 롤링 실행
			   사용처 : pvmFrontScript.comHeaderControl(); */
			...
			if(!$('.hs-prom-roll').hasClass('hide')){
				$('.hs-prom-roll .list').on('init', function(event, slick){
					pvmFrontScript.utilSlickCrt($('.w-header-search .hs-prom-roll .b-slick-crt'), slick); //autoplay control
				}).slick({
					draggable: false,
					arrows:false,
					pauseOnHover: true,
		...
		comGNBInit: function(){
			...
			pvmFrontScript.beforeSettingNav();
		...
		comHeaderControl: function(){
			/* 설명   : 공통 헤더 컨트롤(상단리본, 공지사항, 스크롤 헤더반응형, 검색UI)
			   사용처 : pvmFrontScript.init() 에서 실행 */

			if($('.commonHeaderObject').length > 0){
				//---------------- 헤더 리본
				pvmFrontScript.comTopAreaInit();		

				//---------------- 하단 프로모션
				pvmFrontScript.comBottomPromInit();				

				//---------------- 헤더 롤링		
				pvmFrontScript.onHeaderPromRoll(); 

				//---------------- gnb, 전체메뉴 세팅
				pvmFrontScript.comGNBInit();

				//---------------- 헤더 타입
				pvmFrontScript.comCHOTypeInit();

				//---------------- commonHeaderObject scroll event
				pvmFrontScript.CHOScrollEvent();
			}//..if
		},
		comContents: function(){
			...
			if($('.s-rec-promotion-cont').length > 0){
				...
				srpcRight = $('.srpc-prom .srpc-right .list').on('init', function(event, slick){
					...
					//pvmFrontScript.utilSlickCrt($('.srpc-prom .srpc-right .b-slick-crt'), slick); //autoplay control
				}).slick(sectionMainSlickSettings);
			}
			...
			if($('.s-always-banner').length > 0){
				$('.s-always-banner .ab-list').on('init', function(event, slick){
					pvmFrontScript.utilSlickCrt($('.s-always-banner .b-slick-crt'), slick); //autoplay control
				}).slick({
					draggable: false,
					pauseOnHover: true,
					dots: true,
