...
var pvmSearch = window.pvmSearch || (function(){
	...
	return {
		...
		setAirData: function($area, $idx){
			...
			pvmSearch.changePage('/privia_2018/html_mobile/main_new/air-lpu.html');
		},
		...
		setHotelData: function($area){
			...
			pvmSearch.changePage('/privia_2018/html_mobile/main_new/hotel-lpu.html');
		},
...
/* 
	개편 ver2 
	기존 팝업에서 도시,달력,인원 등등이 분리된 버전
*/
var pvmSearchV2 = window.pvmSearchV2 || (function(){
	var schAirCity = null;
	var schAirCalendar = null;
	var schAirCapacity = null;
	...
	return {
		...
		comSearchAir: function(){
			...
			$('.sc-air .places-exit [data-panel="mainsel"]').on('click', function(e){
				...
				schAirCity = {
					...
					area: 'mainsel-exit',
					places:{
						code: $(this).hasClass('on') ? $('.code', this).text() : '',
						city: $(this).hasClass('on') ? $('.city', this).text() : ''
					}
				}
				pvmSearchV2.changePage('/privia_2018/html_mobile/main_new/air-city-lpu.html');
				e.preventDefault();
			});
			...
			$('.sc-air  [data-panel="calendar"]').on('click', function(e){
				...
				schAirCalendar = {
					...
					area: 'calendar',
					dates: {
						chkin: $(this).hasClass('on') ? $('.qsb-chkin', this).data('day') : '',
						chkout: $(this).hasClass('on') ? $('.qsb-chkout', this).data('day') : '',
					}
				}
				e.preventDefault();
			});
			...
			//인원, 좌석등급
			$('.sc-air  [data-panel="capacity"]').on('click', function(e){
				pvmSearchV2.setAirData('capacity', _idx);
				e.preventDefault();
			});			
		},
		comSearchAirCityLpu: function(){
			var _area = schAirCity.area;
			var _city = schAirCity.places.city;
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 셋팅		
			//출,도착 placeholder 텍스트 셋팅
			if(_area == 'mainsel-exit'){
				$('.sc-search-area .uis-mainsel .uis-input .inp-search').attr('placeholder','어디서 출발 하시나요?');
				pvmFrontScript.changeTitle('출발 도시 선택');
			}
			else if(_area == 'mainsel-entry'){
				$('.sc-search-area .uis-mainsel .uis-input .inp-search').attr('placeholder','어디에 도착 하시나요?');
				pvmFrontScript.changeTitle('도착 도시 선택');
			}
			...
			//해당 도시가 있으면 인풋 셋팅
			if(_city != ''){
				$('.sc-search-area .uis-mainsel .uis-input .inp-search').val(_city);
			}
			...
			$('.sc-search-area .uis-mainsel .uis-list a').on('click', function(e){
				if(!$(this).hasClass('tit-rec')){
					...
					//sbox input 
					schAirCity.$this.addClass('on');
					schAirCity.$this.find('.code').text(code);
					schAirCity.$this.find('.city').text(city);
		...
		comSearchAirCalendarLpu: function(){
			var d;
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 셋팅
			if(schAirCalendar.dates.chkin != ''){
				$('.ui-page-active .uis-datepicker').datepicker('setDate', new Date(schAirCalendar.dates.chkin));
			}
			else if(schAirCalendar.dates.chkout != ''){
				$('.ui-page-active .uis-datepicker').datepicker('setDate', new Date(schAirCalendar.dates.chkout));
			}
		...
		comSearchAirLpu: function(){
			var $currentCity = null; // 도시영역 저장
			var $currentDate = null; // 날짜영역 저장
			var settingData = pvmSearchV2.getSearchData(); 
			var _type; //타입
			var _path = '.sc-search-box.on'; //기본 path
			//다구간 관련
			var _MDMinCnt; //최소 구간
